...
/* include/net/hakc_percpu.h */
#ifndef _NET_HAKC_PERCPU_H
#define _NET_HAKC_PERCPU_H

#include <linux/types.h>
#include <linux/percpu.h>

/* Return a canonical per-CPU base after stripping top-byte tags. */
static __always_inline void __percpu *__hakc_canon_percpu_base(void __percpu *p)
{
    unsigned long v = (unsigned long)p;
    /* strip top byte (TBI/MTE/HAKC) */
    v &= ~(0xFFUL << 56);
    /* for 48-bit VA kernels make sure the top half is all 1s (kernel space) */
    v |= 0xFFFF000000000000UL;
    return (void __percpu *)v;
}

/*
 * Cast back to the original __percpu type.
 * __force silences sparse about address-space casts.
 */
#define HAKC_CANON_PCPU_BASE(p) \
    ((__force typeof(p)) __hakc_canon_percpu_base((void __percpu *)(p)))

/* If you want “strip only, no sign-extension”, use this one instead. */
static __always_inline void __percpu *__hakc_untag_percpu_base(void __percpu *p)
{
    unsigned long v = (unsigned long)p;
    v &= ~(0xFFUL << 56);
    return (void __percpu *)v;
...
#define HAKC_UNTAG_PCPU_BASE(p) \
    ((__force typeof(p)) __hakc_untag_percpu_base((void __percpu *)(p)))

#endif /* _NET_HAKC_PERCPU_H */

#include <linux/bitops.h>   // for sign_extend64()
...
static __always_inline void *hakc_canon48(const void *p)
{
    /* sign-extend from bit 47 to make a canonical 64-bit VA */
    u64 v = (u64)p;
    return (void *)sign_extend64(v, 47);
}

static __always_inline void *hakc_safe_ptr6(const void *p)
{
    return hakc_canon48(hakc_untag(p));
}
...
//#endif
		...
		struct sock **slot_raw = per_cpu_ptr(net->ipv6.icmp_sk, i);
		//struct sock **slot     = (struct sock **)hakc_safe_ptr6(slot_raw);
		__s64 a = (__s64)slot_raw;     // ptr might be 0x02ea… (unauthenticated alias), or even attacker-crafted
		a = (a << 16) >> 16;  // canonicalize to 0xffff…
		struct sock **slot = a;

		// Optional debug to verify the fix 
		pr_info("icmpv6_sk_init: cpu=%d slot_raw=%px slot=%px\n", i, slot_raw, slot);

		// Use the SAFE pointer for the dereference 
		*slot = sk;
