...
//extern struct list_head inetsw6[];
/* Put near other static helpers */
static void dump_inetsw6(void)
{
    int t;
    rcu_read_lock();
    for (t = 0; t < SOCK_MAX; t++) {
        int n = 0;
...
int inet6_register_protosw(struct inet_protosw *p)
{
	...
	for (lh = (&inetsw6[p->type])->next; lh != (&inetsw6[p->type]); lh = lh->next){
		...
		//pr_info("inet6_register_protosw: answer=%px flags=0x%x proto=%d\n",\
			answer, answer->flags, answer->protocol);
