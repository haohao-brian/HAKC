...
{
	if (!list_empty_new(&rt->rt6i_uncached)) {
					 ...
					 int flags)
{
        ...
        if (list_empty_new(&rt6->rt6i_uncached) && !dst_hold_safe(dst)) {
...
static struct dst_entry *ip6_dst_check(struct dst_entry *dst, u32 cookie)
{
	...
	if (from && (rt->rt6i_flags & RTF_PCPU ||
	    unlikely(!list_empty_new(&rt->rt6i_uncached))))
